...
class ContentSafetyFilter:
    ...
    def __init__(self):
        ...
        self.malware_keywords = [
            ...
            'malware', 'virus', 'worm', 'spyware', 'encrypt files'
        ]

        self.phishing_keywords = [
            'phishing email', 'fake login', 'credential harvest',
            'spoof domain', 'fake website', 'password stealing',
            'steal password', 'steal login', 'harvest credential'
        ]

        self.exploit_keywords = [
            'buffer overflow', 'sql injection payload', 'xss attack',
    ...
    def check_request(self, prompt: str) -> List[ContentSafetyAlert]:
        ...
        if any(kw in prompt_lower for kw in self.malware_keywords):
            if any(gen in prompt_lower for gen in ['create', 'generate', 'write', 'build', 'develop']):
                alerts.append(ContentSafetyAlert(
                    severity="critical",
